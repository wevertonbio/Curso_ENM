...
filter_wcvp <- function(occ, species, x, y, wcvp_data, buffer = 50, 
                        return_map = TRUE){
  #Get species in
  all_sp <- unique(occ[[species]])
  sp_in <- intersect(all_sp, wcvp_data$wcp_data$species)
  if(length(sp_in) == 0){
    stop("None of the species in occ are within wcvp_data")
  }
  sp_out <- setdiff(all_sp, wcvp_data$wcp_data$species)
  if(length(sp_out) > 0){
  ...
  occ_filt <- pbapply::pblapply(sp_in, function(i){
      sp_dist <- wcvp_data$wcp_data %>% filter(species == i) %>% pull(LEVEL3_COD)
      m_sp <- wcvp_data$wcp_map[wcvp_data$wcp_map$LEVEL3_COD %in% sp_dist,]
      #Buffer of x km
